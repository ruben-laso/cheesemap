...
namespace chs
{
	...
	class Dense3D
	{
		...
		private:
		static constexpr array_type DEFAULT_RESOLUTIONS = { 1, 1, 1 };
		...
		{
			const auto cell_idx = indices[0] * sizes_[1] * sizes_[2] + indices[1] * sizes_[2] + indices[2];
		...
		Dense3D(Points_rng & points, const resolution_type res) : Dense3D(points, vector_type(Dim, res))
		{}

		template<typename Points_rng>
		Dense3D(Points_rng & points, vector_type res) : resolutions_(std::move(res)), box_(Box::mbb(points))
		{
			...
			for (const auto i : ranges::views::indices(sizes_[0]))
			{
				for (const auto j : ranges::views::indices(sizes_[1]))
				{
					...
					{
						const auto cell_box = idx2box(std::array{ i, j, k });
						cells_.emplace_back(idx2box(std::array{ i, j, k }));
					}
				}
			}
