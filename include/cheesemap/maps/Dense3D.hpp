...
namespace chs
{
	...
	class Dense3D : public chs::Dense<Point_type, 3>
	{
		protected:
		using super_type = chs::Dense<Point_type, 3>;

		[[nodiscard]] inline auto & at(const ranges::range auto & indices)
		{
			const auto cell_idx = indices[0] * this->sizes_[1] * this->sizes_[2] +
			                      indices[1] * this->sizes_[2] + indices[2];
			return this->cells_[cell_idx];
		}
		...
		Dense3D(Args_types &&... args) : super_type(std::forward<Args_types>(args)...)
		{}
