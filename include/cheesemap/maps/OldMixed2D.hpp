...
namespace chs
{
	...
	class OldMixed2D
	{
		...
		OldMixed2D(Points_rng & points, const resolution_type res,
		           const chs::flags::build::flags_t flags = {}) :
		        OldMixed2D(points, dimensions_type(chs::n_tuple<Dim>(res)), flags)
		{}

		template<typename Points_rng>
		OldMixed2D(Points_rng & points, const dimensions_type & res,
		           const chs::flags::build::flags_t flags = {}) :
