...
namespace chs
{
	...
	class OldMixed3D
	{
		...
		OldMixed3D(Points_rng & points, const resolution_type res,
		           const chs::flags::build::flags_t flags = {}) :
		        OldMixed3D(points, dimensions_type(n_tuple<Dim>(res)), flags)
		{}

		template<typename Points_rng>
		OldMixed3D(Points_rng & points, dimensions_type res, const chs::flags::build::flags_t flags = {}) :
		...
		[[nodiscard]] inline auto query(const Kernel_t & kernel, Filter_t && filter) const
		{
			...
			for (const auto k : ranges::views::closed_indices(min_k, max_k))
			{
				...
				{
					const auto * cell_ptr = slice.at(indices);
					if (cell_ptr == nullptr) { continue; }
					for (auto * point_ptr : *cell_ptr)
					{
		...
		[[nodiscard]] inline auto knn(const std::integral auto k_neigh, const Point_type & p) const
		{
			...
			// Explore first the neighbors of the cell containing p
			{
				...
				const auto & cell_ptr = slices_[p_k].at({ p_i, p_j });

				if (cell_ptr)
				{
					ranges::for_each(*cell_ptr, [&](const auto & point) {
