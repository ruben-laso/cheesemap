...
namespace chs
{
	...
	class Sparse2D
	{
		...
		{
			const auto cell_idx = i * sizes_[1] + j;
			return cell_idx - 1;
		}
		...
		[[nodiscard]] auto global_idx_cells_to_search(auto && kernel) const
		{
			...
			const auto [min_i, min_j] = coord2indices(kernel.box().min());
			const auto [max_i, max_j] = coord2indices(kernel.box().max());

			auto indices = std::as_const(indices_).submat(min_i, min_j, max_i, max_j);
		...
		[[nodiscard]] inline auto query(const Kernel_t & kernel, Filter_t && filter) const
		{
			...
			const auto & global_indices = std::as_const(indices_).submat(min_i, min_j, max_i, max_j);
