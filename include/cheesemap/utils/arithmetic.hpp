...
namespace chs
{
	...
	[[nodiscard]] CHSINLINE constexpr auto n_array(const auto & val)
	{
	...
	template<std::size_t N>
	[[nodiscard]] CHSINLINE constexpr auto n_tuple(const auto & val)
	{
	...
	[[nodiscard]] CHSINLINE auto sq_distance(const auto & p1, const auto & p2, std::index_sequence<Is...>) -> double
	{
	...
	[[nodiscard]] CHSINLINE auto sq_distance(const auto & p1, const auto & p2) -> double
	{
		return sq_distance(p1, p2, std::make_index_sequence<Dim>{});
	}

	template<std::size_t Dim = 3>
	[[nodiscard]] CHSINLINE auto distance(const auto & p1, const auto & p2) -> double
	{
		return std::sqrt(sq_distance(p1, p2, std::make_index_sequence<Dim>{}));
	}

	template<std::size_t... Is>
	CHSINLINE void clamp(auto & val, const auto & min, const auto & max, std::index_sequence<Is...>)
	{
		((val[Is] = std::clamp(val[Is], min[Is], max[Is])), ...);
	}

	template<std::size_t Dim>
	CHSINLINE void clamp(auto & vals, const auto & mins, const auto & maxs)
	{
		chs::clamp(vals, mins, maxs, std::make_index_sequence<Dim>{});
	}

	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto max(const auto & a, std::index_sequence<Is...>)
	{
		return std::max({ std::get<Is>(a)... });
	}

	template<std::size_t Dim>
	[[nodiscard]] CHSINLINE auto max(const auto & a)
	{
		return max(a, std::make_index_sequence<Dim>{});
	}

	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto min(const auto & a, std::index_sequence<Is...>)
	{
		return std::min({ std::get<Is>(a)... });
	}

	template<std::size_t Dim>
	[[nodiscard]] CHSINLINE auto min(const auto & a)
	{
		return min(a, std::make_index_sequence<Dim>{});
	}

	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto product(const auto & a, std::index_sequence<Is...>)
	{
		return (std::get<Is>(a) * ...);
	}

	template<std::size_t Dim>
	[[nodiscard]] CHSINLINE auto product(const auto & a)
	{
	...
	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto volume_bbox(const auto & min, const auto & max, std::index_sequence<Is...>)
	        -> double
	{
	...
	template<std::size_t Dim = 3>
	[[nodiscard]] CHSINLINE auto volume_bbox(const auto & min, const auto & max) -> double
	{
		return volume_bbox(min, max, std::make_index_sequence<Dim>{});
	}

	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto volume(const auto & max, const auto & min, const auto sizes,
	...
	[[nodiscard]] CHSINLINE auto radius_for_density(const auto & curr_pts, const auto & curr_r,
	                                                const auto & trgt_pts)
	{
	...
	[[nodiscard]] CHSINLINE auto within_closed_bounds(const T & val, const U & min, const V & max) -> bool
	{
		return std::cmp_less_equal(min, val) and std::cmp_less_equal(val, max);
	}

	[[nodiscard]] CHSINLINE auto within_closed_bounds(const auto & val_min_max) -> bool
	{
	...
	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto within_closed_bounds(const auto & vals, const auto & mins, const auto & maxs,
	                                                  std::index_sequence<Is...>)
	{
	...
	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto all_visited(const auto & mins, const auto & maxs, const auto & sizes,
	                                         std::index_sequence<Is...>)
	{
	...
	template<std::size_t... Is>
	[[nodiscard]] CHSINLINE auto all_equal(const auto & as, const auto & bs, std::index_sequence<Is...>)
	{
		return ((std::get<Is>(as) == std::get<Is>(bs)) and ...);
	}

	template<std::size_t Dim>
	[[nodiscard]] CHSINLINE auto all_equal(const auto & as, const auto & bs)
	{
