...
namespace chs::test
{
	void build_random(auto && map_build_func)
	{
		...
		for ([[maybe_unused]] const auto _ : ranges::views::indices(num_builds))
		{
			const auto min_coord =
			        get_random(chs::mockup::DEFAULT_MIN_COORD - chs::mockup::DEFAULT_MAP_VARIANCE,
			                   chs::mockup::DEFAULT_MIN_COORD + chs::mockup::DEFAULT_MAP_VARIANCE);
			const auto max_coord =
			        get_random(chs::mockup::DEFAULT_MAX_COORD - chs::mockup::DEFAULT_MAP_VARIANCE,
			...
			EXPECT_NO_THROW(const auto map = map_build_func(points));
		}
	}

	void spherical_search(auto && map_build_func)
	{
		...
		for ([[maybe_unused]] const auto _ : ranges::views::indices(num_builds))
		{
			...
			auto map = map_build_func(points);
			...
			for ([[maybe_unused]] const auto i : ranges::views::indices(num_checks))
			{
				...
				const auto results_truth =
				        ranges::views::filter(points,
				                              [&sphere](auto & p) { return sphere.is_inside(p); }) |
	...
	void spherical_search_filter(auto && map_build_func)
	{
		static constexpr std::size_t num_builds = 20;
		static constexpr std::size_t num_points = 10'000;
		static constexpr std::size_t num_checks = 20;
		...
		for ([[maybe_unused]] const auto _ : ranges::views::indices(num_builds))
		{
			...
			for ([[maybe_unused]] const auto i : ranges::views::indices(num_checks))
			{
				...
				const auto filter = [](auto & p) { return p[0] > 50.0; };

				const auto results_map = map.query(sphere, filter);

				const auto results_truth =
				        ranges::views::filter(
				                points, [&](auto & p) { return sphere.is_inside(p) and filter(p); }) |
				        ranges::views::addressof | ranges::to<std::vector>();
		...
		for ([[maybe_unused]] const auto _ : ranges::views::indices(num_builds))
		{
			...
			for ([[maybe_unused]] const auto i : ranges::views::indices(num_checks))
			{
				...
				auto get_truth = [&]() {
					...
					ranges::actions::sort(dist_points, [](const auto & a, const auto & b) {
