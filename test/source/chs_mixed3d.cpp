...
TEST(chs, chs_mixed3d_build)
{
	auto points = chs::mockup::naive_points();

	EXPECT_NO_THROW(const auto map = chs::Mixed3D<chs::Point>(points, CELL_SIZE));
}

TEST(chs, chs_mixed3d_build_random)
{
...
TEST(chs, chs_mixed3d_spherical_search)
{
	...
	for ([[maybe_unused]] const auto _ : ranges::views::indices(num_builds))
	{
		...
		auto map = chs::Mixed3D<chs::Point>(points, CELL_SIZE);
...
TEST(chs, chs_mixed3d_spherical_search_filter)
{
	static constexpr std::size_t num_builds = 20;
	static constexpr std::size_t num_points = 10'000;
	static constexpr std::size_t num_checks = 20;
