...
using map_type = chs::Sparse<chs::Point, 3>;

TEST(chs, chs_sparse_build)
{
	auto points = chs::mockup::naive_points();

	EXPECT_NO_THROW(const auto map = chs::Sparse<chs::Point>(points, CELL_SIZE));
}

TEST(chs, chs_sparse_build_random)
{
...
TEST(chs, chs_sparse_spherical_search)
{
...
TEST(chs, chs_sparse_spherical_search_filter)
{
	static constexpr std::size_t num_builds = 20;
	static constexpr std::size_t num_points = 10'000;
	static constexpr std::size_t num_checks = 20;
